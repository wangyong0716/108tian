...
import com.ksider.mobile.android.view.materialcalendarview.format.WeekDayFormatter;
...
class MonthView extends LinearLayout implements View.OnClickListener {
    ...
    protected List<DayViewSelectable> mSelectables;
    ...
    public MonthView(Context context, CalendarDay month, int firstDayOfWeek, boolean hasBorderLines) {
        super(context);
        this.month = month;
        ...
        for (int r = 0; r < DEFAULT_MAX_WEEKS; r++) {
            if (hasBorderLines) {
                TextView horLine = new TextView(context);
                horLine.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, 1));
                horLine.setBackgroundColor(context.getResources().getColor(R.color.divider));
                addView(horLine);
            }
            ...
            for (int i = 0; i < DEFAULT_DAYS_IN_WEEK; i++) {
                ...
                if (i != DEFAULT_DAYS_IN_WEEK - 1 && hasBorderLines) {
                    TextView verLine = new TextView(context);
                    verLine.setLayoutParams(new LayoutParams(1, LayoutParams.MATCH_PARENT));
                    verLine.setBackgroundColor(context.getResources().getColor(R.color.divider));
                    row.addView(verLine);
                }
                calendar.add(DATE, 1);
            }
        ...
        if (hasBorderLines) {
            TextView lastLine = new TextView(context);
            lastLine.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, 1));
            lastLine.setBackgroundColor(context.getResources().getColor(R.color.divider));
            addView(lastLine);
        }
    ...
    public MonthView(Context context, CalendarDay month, int firstDayOfWeek) {
        this(context,month,firstDayOfWeek,false);
    }
    ...
    void setDayViewSelectable(List<DayViewSelectable> selectables) {
    ...
    protected void updateSelectable() {
        if (mSelectables != null) {
            for (DayView dayView : monthDayViews) {
                for (DayViewSelectable selectable : mSelectables) {
                    dayView.setEnabled(selectable.selectable(dayView.getDate()));
                }
            }
        }
    }

    protected void addMoney(HashMap<CalendarDay, Double> dates) {
        if (dates != null) {
            for (DayView dayView : monthDayViews) {
                if (dates.keySet().contains(dayView.getDate())) {
                    dayView.addMoney(dates.get(dayView.getDate()));
                }
            }
        }
    }
