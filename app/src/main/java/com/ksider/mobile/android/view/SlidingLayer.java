...
public class SlidingLayer extends FrameLayout {
	...
	private int mFilterOffsetWidth = 0;
	private View mPanel;
...
//					if (parent.getChildAt(index).getId() == R.id.mask) {
//						mPanel = parent.getChildAt(index);
//					}
				}
			}
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (screenSide != mScreenSide || mForceLayout) {
			...
			if (mScreenSide == STICK_TO_RIGHT) {
				setPadding(getPaddingLeft() + mShadowWidth + mOffsetWidth, getPaddingTop(), getPaddingRight(),
				        getPaddingBottom());
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mShadowWidth > 0 && mShadowDrawable != null) {
			if (mScreenSide == STICK_TO_RIGHT) {
				mShadowDrawable.setBounds(mOffsetWidth, 0, mOffsetWidth + mShadowWidth, getHeight());
			}
	...
	public void drawFade(Canvas canvas) {
		if (!mHasOpened)
			return;
		float openPercent = (float) ((getWidth() + mScroller.getCurrX() - mOffsetWidth) * 1.0 / (getWidth() - mOffsetWidth));
		final int alpha = (int) (0.7 * 255 * Math.abs(openPercent));
		int left = 0;
		int right = 0;
		if (mScreenSide == STICK_TO_RIGHT) {
			left = 0;
			right = mOffsetWidth + mShadowWidth;
		} else if (mScreenSide == STICK_TO_LEFT) {
			left = getWidth() - mOffsetWidth - mShadowWidth;
			right = getWidth();
		}
		if (mScroller.isFinished() && !isOpen()) {
			mPanel.setVisibility(View.INVISIBLE);
		} else {
			mPanel.setVisibility(View.VISIBLE);
			if (mPanel != null) {
				ColorDrawable cd = new ColorDrawable(R.color.color_black);
				cd.setAlpha(alpha);
				mPanel.setBackgroundDrawable(cd);
			}
		}
