...
public class EasySSLSocketFactory implements LayeredSocketFactory {
    ...
    public EasySSLSocketFactory(Context context, boolean isConnectingToGoinoutServer) {
        mContext = context;
     ...
     */
    private static SSLContext createEasySSLContext(Context AppContext) throws IOException {
        try {
        	
            // Client should authenticate itself with the valid certificate to Server.
        	InputStream clientStream = AppContext.getResources().openRawResource(R.raw.production_client);
	        ...
	        instream = AppContext.getResources().openRawResource(R.raw.production_ca);
