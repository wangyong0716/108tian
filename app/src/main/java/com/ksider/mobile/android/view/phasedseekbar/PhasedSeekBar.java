...
public class PhasedSeekBar extends View {
	...
	protected Drawable mCursorIcon;
	...
	protected void init(AttributeSet attrs, int defStyleAttr) {
		...
		if (attrs != null) {
			...
			mCursorIcon = a.getDrawable(R.styleable.PhasedSeekBar_phased_base_cursor);
			a.recycle();
		}
	}

	protected void configure() {
		Rect rect = new Rect((int) mBackgroundPaddingRect.left, (int) mBackgroundPaddingRect.top, (int) (getWidth() - mBackgroundPaddingRect.right),
		...
		int count = getCount();
		int paddingWidth = mItemHalfWidth > mItemAnchorHalfWidth ? mItemHalfWidth : mItemAnchorHalfWidth;
		int paddingHeight = mItemHalfHeight > mItemAnchorHalfHeight ? mItemHalfHeight : mItemAnchorHalfHeight;
		int widthBase = count > 1 ? (getWidth() - 2 * paddingWidth) / (count - 1) : getWidth();
		int widthHalf = widthBase / 2;
		int heightBase = count > 1 ? (getHeight() - 2 * paddingHeight) / (count - 1) : getHeight();
		int heightHalf = heightBase / 2;
		mAnchors = new int[count][2];
		for (int i = 0; i < count; i++) {
			mAnchors[i][0] = mModeIsHorizontal ? widthBase * i + paddingWidth : mPivotX;
			mAnchors[i][1] = !mModeIsHorizontal ? heightBase * i + paddingHeight : mPivotY;
		}
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < count; i++) {
			...
			itemOff.setBounds(mAnchors[i][0] - mItemHalfWidth, mAnchors[i][1] - mItemHalfHeight, mAnchors[i][0] + mItemHalfWidth, mAnchors[i][1]
					+ mItemHalfHeight);
			itemOff.draw(canvas);
		}

		if (mCursorIcon != null) {
			mCursorIcon.setBounds(mCurrentX - mItemAnchorHalfWidth, mCurrentY - mItemAnchorHalfHeight, mCurrentX + mItemAnchorHalfWidth, mCurrentY
					+ mItemAnchorHalfHeight);
	...
	protected int getNormalizedX(MotionEvent event) {
		int paddingWidth = mItemHalfWidth > mItemAnchorHalfWidth ? mItemHalfWidth : mItemAnchorHalfWidth;
		return Math.min(Math.max((int) event.getX(), paddingWidth), getWidth() - paddingWidth);
	}

	protected int getNormalizedY(MotionEvent event) {
		int paddingHeight = mItemHalfHeight > mItemAnchorHalfHeight ? mItemHalfHeight : mItemAnchorHalfHeight;
		return Math.min(Math.max((int) event.getY(), paddingHeight), getHeight() - paddingHeight);
	}
